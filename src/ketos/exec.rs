...
pub fn execute(ctx: &Context, code: Arc<Code>) -> Result<Value, Error> {
    ...
    mach.execute(ctx.scope(), code).inspect_err(|e| {
...
pub fn call_function(ctx: &Context, fun: Value, args: Vec<Value>) -> Result<Value, Error> {
    match fun {
        Value::Function(fun) => execute_function(ctx, fun, args).inspect_err(|e| {
...
pub fn execute_lambda(ctx: &Context, lambda: Lambda, args: Vec<Value>) -> Result<Value, Error> {
    ...
    mach.execute_lambda(lambda, args).inspect_err(|e| {
        set_traceback(mach.build_trace());
    })
}
...
impl Machine {
    ...
    fn drain_stack_top(&mut self, n: u32) -> Result<Drain<'_, Value>, ExecError> {
        let len = self.stack.len();
