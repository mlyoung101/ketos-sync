...
pub fn highlight_span(text: &str, span: Span) -> SpanDisplay<'_> {
    ...
    SpanDisplay {
        ...
        highlight: std::iter::repeat_n(' ', pre_chars)
            .chain(once('^'))
            .chain(std::iter::repeat_n('~', span_chars.saturating_sub(1)))
            .collect(),
    }
}
...
impl CodeMap {
    ...
    ///
    /// Panics if `span` is not valid.
    pub fn highlight_span(&self, span: Span) -> SpanDisplay<'_> {
        let n = match self.files.binary_search_by(|f| f.begin.cmp(&span.lo)) {
            Ok(n) => n,
            Err(n) => n - 1,
        };

        let end = match self.files.get(n + 1) {
            Some(f) => f.begin,
...
fn parse_doc_comment(input: &str) -> (Token<'_>, usize) {
...
fn parse_keyword(input: &str) -> Result<(Token<'_>, usize), ParseErrorKind> {
...
fn parse_name(input: &str) -> Result<(Token<'_>, usize), ParseErrorKind> {
    parse_identifier(input).map(|(ident, size)| (Token::Name(ident), size))
}

fn parse_number(input: &str) -> Result<(Token<'_>, usize), ParseErrorKind> {
    ...
    let (base, prefix_offset, rest) = if input.starts_with("0x") {
        ...
        match input[1..].chars().next() {
            Some(ch) if ch.is_ascii_digit() => (10, 1, &input[1..]),
...
fn parse_byte(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_byte(input, pos)?;
    Ok((Token::Byte(&input[..size]), size))
}

fn parse_bytes(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_byte_string(&input[2..], pos + 2)?;
    Ok((Token::Bytes(&input[..size + 2]), size + 2))
}

fn parse_raw_bytes(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_raw_byte_string(&input[2..], pos + 2)?;
    Ok((Token::Bytes(&input[..size + 2]), size + 2))
}

fn parse_path(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_string(&input[2..], pos + 2)?;
    Ok((Token::Path(&input[..size + 2]), size + 2))
}

fn parse_raw_path(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_raw_string(&input[2..], pos + 2)?;
    Ok((Token::Path(&input[..size + 2]), size + 2))
}

fn parse_char(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_char(input, pos)?;
    Ok((Token::Char(&input[..size]), size))
}

fn parse_string(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
    let (_, size) = string::parse_string(input, pos)?;
    Ok((Token::String(&input[..size]), size))
}

fn parse_raw_string(input: &str, pos: BytePos) -> Result<(Token<'_>, usize), ParseError> {
