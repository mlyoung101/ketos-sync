...
pub fn compile(ctx: &Context, value: &Value) -> Result<Code, Error> {
    ...
    compiler.compile(value).inspect_err(|e| {
...
impl<'a> Compiler<'a> {
    ...
    fn take_trace(&mut self) -> Trace {
        Trace::new(std::mem::take(&mut self.trace), self.trace_expr.take())
    }
    ...
    fn expand_macro(&mut self, name: Name, args: &[Value], expr: &Value) -> Result<Value, Error> {
        ...
        execute_lambda(&self.ctx, lambda, args.to_vec()).inspect_err(|e| {
    ...
    fn eval_constant(&mut self, value: &Value) -> Result<ConstResult, Error> {
        match *value {
            ...
            Value::List(ref li) => {
                ...
                self.eval_constant_function(name, &li[1..]).inspect_err(|e| {
                    self.set_trace_expr(value);
                })
            }
...
fn op_use(compiler: &mut Compiler, args: &[Value]) -> Result<(), Error> {
    ...
    let m = mods.load_module(mod_name, &ctx).inspect_err(|e| {
